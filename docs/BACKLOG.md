...
| BL-116 | **In-app App tickets** | **Locked A 2026-08-19.** Green **Raise ticket** (global + form chip) → Hub **App tickets** tab. GREEN-note lifecycle (Log Note / Defer / Resolve). Dedicated `app_tickets` + `app_ticket_notes` — not Maintenance or Human Incidents. Auto context: staff, route, form title, last tap, event/site/vehicle, SIM, TEST/PROD. **Notify (2026-08-19):** Dashboard Band 3 **App tickets** tile (all open/in-progress, not stale-only) → Hub `?tab=app_tickets`; server **Postmark** email via Admin To (`app_tickets.notify_to`) + `/api/internal/app-ticket-notify`. SQL: `docs/sql/2026-08-19_app_tickets.sql` + `docs/sql/2026-08-19_app_ticket_notify.sql`. Host env: `POSTMARK_SERVER_TOKEN` + verified From (`POSTMARK_FROM` or Admin From). Not Council mailto (BL-062). | **ready** |
