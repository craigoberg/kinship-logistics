...
| BL-002 | **Security RBAC mode** | **Menus / idle lock still last.** **2026-08-20 data-plane slice (BL-117):** operational tables require day-login JWT; anon only for published CMS + `submit_public_form`. **Remaining:** idle PIN lock (exempt active Manifest); menu/`role_menu_access`; Hub/Admin gates; trip leader vs manager. **Do not** re-open anon ALL on PII tables. Target two-tier: (1) **Day session** — email + password (**built**). (2) **Screen lock** — idle timeout; same staff PIN via `PinReauthDialog`; **exempt active manifest**. (3) **Action step-up** — existing `PinEntryDialog`. **Build constraints:** GUARDRAILS §2.4 + `.cursor/rules/rbac-forward-compat.mdc`. | **deferred** (menus last; RLS slice → BL-117) |
