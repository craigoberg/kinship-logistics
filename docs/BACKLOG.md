...
| BL-099 | **Environments: DEV → TEST → PROD promotion** | **In progress 2026-07-29.** Thin day login on DEV. TEST Supabase (AU) + **Vercel** host proven (DEV env first). Runbook: `docs/architecture/ENV-LANES.md`. **Now:** point Vercel at TEST Supabase, restore JSON, Auth users, Alpha smoke. PROD = 2nd Vercel project later. Lovable abandoned as host. Full RBAC stays BL-002. | **ready** (Vercel up; cut over to TEST DB) |
