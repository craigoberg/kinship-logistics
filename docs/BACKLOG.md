...
| BL-099 | **Environments: DEV → TEST → PROD promotion** | **In progress 2026-07-29.** Thin day login shipped on DEV. **TEST Supabase project stood up (Australia).** Runbook: `docs/architecture/ENV-LANES.md` (backup DEV → restore TEST → Auth users on TEST → Lovable TEST env keys + `VITE_APP_LANE=test`). **Still:** populate TEST schema/data; wire TEST publish env; Alpha smoke; PROD later. Full idle lock / menu RBAC stays BL-002. | **ready** (TEST project up; load + wire next) |
...
| BL-092 | **Policies & Procedures library** | Practice Standards–aligned org policies (incident, complaints, privacy, HR, etc.): review drafts, version control, SharePoint home (**BL-051**), map to standards. **Audit Pack:** approved set dumps into ZIP folder `05_Policies_Procedures/` (currently stub README only). Distinct from Hub compliance certs (`04_`) and from onboarding Word packs (BL-065 → `06_`). **Includes belongings/valuables SOP (BL-079)** — policy-first; no dedicated custody UI unless losses force it. | **discuss** |
...
| ID | Item | Notes | Status |
|----|------|-------|--------|
| BL-062 | **Council escalations & management** | **Locked + mailto path shipped 2026-07-28.** Escalate → pre-fill **mailto** (user mail client) → log email_dispatched_to_council + ledger. Hub escalate opens mail when council_email_to set. Follow-up = Hub notes; stale RYG timers. **No** server SMTP/Postmark/Lovable send. DEV/TEST To = internal param; PROD = real council. **Later discuss:** which types are council-eligible; response/ack workflow; reporting. | **ready** (mailto done; workflow polish later) |
...
| BL-084 | **Infection control, site close & emergencies** | **Design locked 2026-07-27 (A→B→C).** **Phase A + A.1 built** — dual entry Day Centre + Event Deliver; if in care → home-safe disposition (family/staff/taxi/other) + handover + Manager PIN (outcome, not route); if not present → exclude only; Hub Clear to return later. SQL: 2026-07-27_infectious_exclusion_phase_a.sql + …_home_safe.sql. **Still open:** (B) site close; (C) emergency activate / muster / stand-down (**Drill vs Live**). **Office capture (2026-07-28 — before full B/C plan):** meet office → inventory what they already do (expect little) → fold existing practice into Yada; if gaps, design from MVP below + docs review (BL-092). **Ask office:** written procedures today (fire, medical, missing person, lockdown, bomb, weather, bus/trip); cadence + last dates; paper/tabletop vs physical; who may declare / sign; where evidence is filed; PEPs for non-independent evacuators; centre-only vs trip-specific. **Drill types to consider (not all need app types):** fire/evac (core); medical emergency (core); missing person (ops RED exists — ask if they also drill); infectious/outbreak (A + site close B); severe weather/flood/heat/power (often B not muster); bomb/suspicious package (often tabletop); armed intruder/lockdown (staff paper only — no client “shooter” sims); robbery only if cash on site (else N/A via BL-083); gas/kitchen if relevant; bus/trip emergency (Manifest/Event Deliver). **Phase C MVP (paper-first OK):** manager activate on Day Centre / Event Deliver / Manifest → type + **Drill vs Live** + PIN → light muster/account (not forced park evacuation) → notify (mock → BL-050) → stand-down + debrief → Hub Human / Health & Safety + ledger → Audit Pack. Cadence reminders can use Compliance Asset due dates today; C is the *run + evidence*. Disability reality: full physical evac with large high-support groups often impractical — auditors care procedure + practised + recorded + PEPs; occasional staff-only / small-group physical if office says safe. **Not MVP:** client shooter/robbery simulations; separate module per hazard; SMS-dependent drills. Full B/C build plan after office answers land. | **ready** (A/A.1 done; B/C next — office input pending) |
