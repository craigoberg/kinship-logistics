...
**Local checkout:** `C:\Users\Craig\kinship-logistics` only. See `docs/LOCAL-WORKSPACE.md`. Do not edit the OneDrive copy.
...
| BL-051 | **SharePoint integration setup** | GUARDRAILS §5.2 — compliance PDFs, vehicle servicing slips, certification docs, signed onboarding packs live on SharePoint; app stores links + Hub expiry. **Locked direction 2026-08-10:** do **not** use Vercel disk as the office file share. **Taxonomy draft:** `docs/architecture/SHAREPOINT-FOLDER-TAXONOMY.md` (libraries 01–07 + person folders). **Build (Graph/app registration/sync) stays deferred** until filing pain warrants it; office can create the folder tree anytime. Public website images = BL-113 (separate). Also required for BL-065 archive + BL-092 P&P. | **discuss** (taxonomy drafted; build later) |
...
| BL-099 | **Environments: DEV → TEST → PROD promotion** | **Alpha host live 2026-07-29** — `crm-test.yada.org.au` + TEST Supabase; SSL via Vercel. **Schema baseline synced DEV↔TEST 2026-08-05.** Prefer **Admin backup v2** (BL-057) for structure+data promote with mode switches; keep dated `docs/sql/` for incremental changes. PROD create = v2 Infrastructure restore and/or regenerated schema pack, then forward migrations. Runbook: `docs/architecture/ENV-LANES.md` (includes TEST host password). Code promote: merge `main`→`test`. Menu RBAC=BL-002 last. Data-plane lock=BL-117. | **ready** (TEST live; baseline sync done) |
| BL-117 | **Day-login operational RLS** | **SQL + code 2026-08-20.** Revoke `anon` on operational/PII tables; `authenticated` CRUD after day login; public CMS SELECT (published/visible only); `submit_public_form` SECURITY DEFINER for Hub tickets; backup/SMS/ticket-notify use service_role. SQL: `docs/sql/2026-08-20_day_login_operational_rls.sql`. Host password on TEST is office/Vercel (ENV-LANES). Idle lock still BL-002. | **ready** (await DEV+TEST SQL + host password) |
...
| BL-120 | **Office event live watch** | **Built 2026-08-22.** Event Manage **Live** tab — read-only projection of Group Status, named Transport IN pickups (`trip_legs`), venue attendance, programme stops, hops/HOME boarding, morning/evening rolls, open issues. No floor taps. Deep-link `?manage=<id>&tab=live`. | **ready** |
| BL-121 | **One Manifest per run slot** | **Built 2026-08-23.** Cannot open a second event IN/HOME or Day Centre morning/afternoon while that slot is live or already closed. Same staff resumes. Named error. Manager incident override (smash-the-bus) **not** built yet. SQL `docs/sql/2026-08-23_transport_run_slot_exclusivity.sql`. | **ready** |
...
| BL-097 | **Day Centre floor visitors (non-registered)** | **Done 2026-07-26 (SIM smoke).** Day Centre only: non-registered floor visitors (`site_day_visitors`); Close Centre blocks while present. Event floor add forbidden — see **BL-098**. **2026-08-02:** kinds = trial · friend_family · site · other (Friend+Family merged; Site visitor added). SQL: `docs/sql/2026-07-26_site_day_visitors.sql`, `docs/sql/2026-08-02_site_day_visitor_kind_site.sql`. | **done** |
| BL-098 | **Event planned guest / bring-a-friend** | **Done 2026-07-26 (SIM smoke).** Guests = real `participants` (`participant_kind=guest`) + `event_roster_bookings` (`is_guest_booking`). Event Manage Roster **Add guest**; reuse/archive lifecycle; non-NDIS `Private` funding; host link; Confirm amber / Open location hard-block. DOB uses `getDobDatePickerProps()`. **Close event** silently archives that trip’s guests (skips if still on Open/Confirmed). Day Centre visitor **Add to event…** → pick event → Add guest prefilled. SQL: `docs/sql/2026-07-26_event_guest_participants.sql`. Unplanned night-of walk-on → **BL-122**. | **done** |
| BL-122 | **Unplanned walk-on (guest / client / carer)** | **Locked 2026-08-23.** Same action, two quiet doors: Manifest **plus icon** beside Absent on the active stop card (this stop, no new geographic leg) and Event Deliver compact **Someone extra arrived** under the Check-In list (self at venue). Driver `PinEntryDialog`; informal leader call not logged. **YELLOW** `[WALK-ON]` Hub issue + canned workaround. Pick client / prior guest / host carer, or new guest/carer. Minimal intake (name, host, allergies, optional phone, return bus/self). Walk-ons skip Open-location incomplete-guest hard-block. Badge **Walk-on · Intake incomplete**. SQL `docs/sql/2026-08-23_event_walk_on.sql`. | **ready** (DEV SQL done; run same file on TEST with this promote) |
...
| BL-092 | **Policies & Procedures library** | **Catalogue locked 2026-08-11** (clean slate; AI Folder_1–3 + office CORE.zip = reference only). **Scope:** registration groups **0136 / 0125 / 0108 only**. **Evidence rule:** Connect primary · paper fallback · SharePoint signed PDFs (`05_Policies_Procedures` via BL-051). **Master policies:** Rights · Privacy (+ data-breach procedure) · Safeguarding · Governance · Risk & Emergency · Quality/CI · Complaints · Incidents · Workforce screening/induction · Intake & service planning · Safe environments / infection · Medication & mealtime · **Conflict of interest** · **Code of conduct** · **Restrictive practices stance** (no RP; do not implement BSPs) · **Money handling (BL-083)** · **Belongings (BL-079)** · **Exit / transition**. **Handbooks:** participant + staff/volunteer (rewrite). **Templates:** service agreement, consents/media, advocacy, CoI declaration, exit form — align BL-065 where overlapping. **File-evidence programmes:** BL-114 (client support/risk/comms) · BL-115 (workforce HR evidence). **Not separate policies:** COVID library, CI register module, emergency pack duplicates (absorb into POL_05 + BL-084). **Rewrite order:** Incidents → Complaints → Privacy → Safeguarding → Workforce → Intake → Meds/Meals → Risk/Emergency → rest. **Full DRAFT pack v0.1 (2026-08-11)** in `05_Policies_Procedures/_Drafts/`: 18 policies + 2 handbooks + evidence templates + **Ops Document Control Go-Live Guide** (tags: SP-PDF · ONBOARD · CONNECT · HUB · OFFICE-REG · PUB-WEB). Board review/sign-off → `_Published/`. Audit Pack ZIP `05_` fills when approved. Service agreement commercial rewrite still office/legal pass. | **ready** (drafts for review) |
...
| BL-116 | **In-app App tickets** | **Done 2026-08-20 (TEST).** Green **Raise ticket** (global + form chip) → Hub **App tickets** tab. GREEN-note lifecycle (Log Note / Defer / Resolve). Dedicated `app_tickets` + `app_ticket_notes` — not Maintenance or Human Incidents. Auto context: staff, route, form title, last tap, event/site/vehicle, SIM, TEST/PROD. Dashboard Band 3 **App tickets** tile → Hub `?tab=app_tickets`. Server **Postmark** email via Admin To/From + `/api/internal/app-ticket-notify`. SQL: `docs/sql/2026-08-19_app_tickets.sql` + `docs/sql/2026-08-19_app_ticket_notify.sql`. Host env: `POSTMARK_SERVER_TOKEN` + verified From. Not Council mailto (BL-062). | **done** |
...
| BL-106 | **Self-administered meds + staff medical assist** | **Locked 2026-08-02.** (1)–(3) self-med profile/manifest still to build. (4)–(6) **Occurred-at + Human who-fields shipped 2026-08-02:** Big Red Human/Asset + Log Anomaly require Occurred at; Human requires client(s) or “No client” + assisting staff; Hub/Maintenance show Occurred + Logged; SQL `docs/sql/2026-08-02_occurred_at_incident_human.sql`. Remaining: client `staff`/`self` med flag + manifest awareness chips. | **ready** (intake/timestamps done; med arrangement later) |
| BL-078 | **End-of-day / shift handover** | Short structured handover: who is still on site, open issues, meds/meals outstanding, notes for next shift / office. **Pairs with** Hub stale SLA (BL-066) and Close Centre (BL-075). **Read-only End of Day Report shipped 2026-08-26 (BL-123)** — this item remains the PIN/sign-off handover form. **Discuss:** Day Centre vs trip-day; required fields vs free note; PIN sign-off. | **discuss** |
| BL-079 | **Belongings & valuables** | **Locked under BL-092 (2026-08-11).** Policy: participants retain own valuables; staff do not hold participant cash/phones/keys as custody. Lost property → Hub issue. **No** custody/locker UI unless ops force it. Org petty cash is **BL-083** (separate from participant property). | **deferred** (→ BL-092 policy write) |
| BL-080 | **Consent / contact currency** | **Absorbed into BL-065 ALPHA (2026-08-10).** Client consent pack Hub asset (`client_consent_pack`, +12 months) created on Signed & filed; Review/Update resets dates. Floor warn/block on stale consent still later. SharePoint artefacts remain BL-051. | **done** (→ BL-065 Hub currency; floor gates later) |
| BL-081 | **Staffing vs headcount** | Light check that floor/trip staffing matches rostered participant numbers / ratio expectations for that day. **Not** full rostering/HR. **Prerequisite:** **BL-125** support-people presence (staff / volunteer / carer counts). **Discuss:** ratio rules source; warn vs block Open; multi-bus / multi-venue split. Do **not** invent ratio rules until BL-125 counts are live. | **discuss** |
| BL-125 | **Day & trip support people (staff / volunteer / carer)** | **Locked 2026-08-29.** Office asked that everyone attending on the day is recorded — staff, volunteers, carers — and that they can be planned onto Day Centre and trip buses with **their own pickup/drop-off methods** (own address, own run, own Manifest stop), same as a client. Identity stays on `staff_registry` / `carers_registry` — **do not** fake `participants` rows (meals / meds / NDIS billing stay client-only). Casual tradies stay `site_day_visitors`. Driver / trip leader already assigned = present; extra stop only if they also ride as a passenger. **IA (same day):** Participants and Staff only set **defaults** (IN/OUT + days). Office consolidates everyone on **Run Planning** (`/run-planning`, between Staff and Transport) — attendance-style board + drag-order runs. Event Manage roster always lists staff / volunteer / carer transport. **Slices:** (1) weekly/trip plan + Manifest legs; (2) Support check-in (Day + Event Deliver) + EOD (BL-123) / Trip Report / audit pack names; (3) **Floor parity 2026-08-29** — no-show sweep, defer/clock, PIN absent, late arrival, Off today Manifest skip, event Group status + morning/evening roll include support. SQL: `docs/sql/2026-08-29_support_people_attendance.sql` + `docs/sql/2026-08-29_support_floor_parity.sql`. Meals/meds stay client-only. (4) BL-081 ratio later. Cross-links: BL-081 · BL-123 · BL-069 · BL-122. | **ready** |
| BL-128 | **Run Planning change log** | **Built 2026-09-18; 5W Activity log 2026-09-18.** Admin **Activity log** is a read-only tab over `operational_ledger` (plus meal-serve / medication-dose overlays). Columns When / Who / What / **Where** / **Why** / Type (Type is a filter, not a place). GPS prints under Where when captured. Check-in/out, emergency/close/lockdown, trip rolls, hops, med-bag, and infectious exclusion name person, place, and reason. Automated Yellow/Red sweeps actor as **System**. Cap 2000 rows per range. Human office saves write `OFFICE_RECORD_CHANGED`. Floor YELLOW/RED keep existing ledger rows. Cursor rule `.cursor/rules/human-activity-log.mdc`. SQL `docs/sql/2026-09-18_compliance_asset_ledger_summary.sql`. Historical Jenny McAlees rows still have time only. | **ready** |
| BL-130 | **Pickup address book** | **Built 2026-09-23.** Home stays the street address. Named places on the client, staff, and carer record. Run Planning picks the standing place per weekday, morning and afternoon separately. Stop order stays one list per run. Driver can switch a pending stop for that run only (saved place or a one-off). Events copy a place into the existing event pickup text. SQL `docs/sql/2026-09-23_person_addresses.sql`. | **ready** (await SQL) |
| BL-129 | **Service exit (off-board clients and staff)** | **Built 2026-09-23.** Clients: `participants.service_status` active/exited plus exit stamps. Staff/volunteers: `staff_registry.active=false` plus the same stamps. History stays. Future centre schedules, medication (Management Operational Directive), and non-live bookings stop. Refuses while checked in, on a live manifest, trip leader, or booked on an Open event that has not ended. Deceased cannot be reactivated. Reactivate does not restore plans. Day login and PIN stay blocked while inactive. Guests still use Archive guest. Carers are not off-boarded. Paper exit form stays BL-092. SQL `docs/sql/2026-09-23_service_exit.sql`. | **ready** |
| BL-083 | **Money handling (centre + trips)** | **Practice locked 2026-08-11.** Debit card is primary spend path. **Small on-site org petty cash** kept for incidentals that cannot go on the card; secured/hidden when site unattended. Receipts + simple log required (policy under BL-092; board/office own the float). **Not** participant personal-money custody (that stays refused — see BL-079). Trip expenses / Trip Report P&L / vendors (BL-001) unchanged. App float UI **not** required unless logging pain appears. | **ready** (policy write via BL-092; no app build yet) |
| BL-084 | **Infection control, site close & emergencies** | **Design locked 2026-07-27 (A→B→C).** **Phase A + A.1 done.** **Phase B + C MVP built 2026-07-29.** **Entry locked 2026-07-29:** Big Red third lane **Health & Safety** → `GlobalHealthSafetyFlow` (everywhere); no duplicate H&S/Emergency chips on Day/Event/Manifest. **B:** do-not-open; lockdown/early close; programme suspend. **C:** Drill\|Live activate + sticky banner + light muster + stand-down. **Stand-down does not auto-resolve Hub.** SQL: `2026-07-29_operational_emergencies_mvp.sql`. **Floor shell 2026-08-06:** global `FloorAnnouncementStrip` on all AppShell pages; Dashboard informational `EmergencyFloorAlert` (Yellow=STANDBY / Red=EVACUATE TO MUSTER POINT; Live pulses, Drill solid); MOTD via Admin `floor_motd` (`2026-08-06_floor_motd_system_parameter.sql`). Priority: Emergency > (future Med alert) > MOTD. Notify = mock SMS (BL-050). Later: Policies (BL-092); Hub print **BL-104**; med-alert channel on same floor stack. | **ready** (A–C MVP; global floor + MOTD 2026-08-06) |

---


## Public website & rights/voice forms

Locked 2026-08-10: **yada.org.au** = public site (CMS from Connect); **connect.yada.org.au** = CRM/ops. All public submissions (incl. anonymous) create **Governance Hub** tickets (never email-only).

| ID | Item | Notes | Status |
|----|------|-------|--------|
| BL-110 | **Public website + CMS** | Content for `yada.org.au` authored/published in Connect Admin → Public website. Tables: `cms_pages`, `cms_nav`, `cms_media`, publish snapshot. Public routes under `/public/*` (no AppShell / no day-login). Domain cutover: public app on yada.org.au, Connect on connect.yada.org.au. **Content pack v1:** `docs/sql/2026-08-10_public_cms_content_v1.sql`. **Office editor 2026-08-22:** `CmsRichTextEditor` (Visual + HTML) — no raw-HTML-only editing. Branding colours/logo = BL-113. File upload = BL-119. Policies public subset; full P&P = BL-092 + SharePoint (BL-051). | **ready** (content v1; office editor shipped) |
| BL-111 | **Public forms → Hub** | Fixed forms: Complaint (anonymous OK), Enquiry, Feedback, Compliment, Volunteer EOI. `public_form_definitions` + `public_form_submissions`; each submit creates `operational_incidents` tagged `[PUBLIC FORM · …]` + reference. Hub card: indigo **Public web** (not Incident); Issue = form type + ~140 char message; complaints stay Yellow without empty Workaround. No IP stored. SQL `docs/sql/2026-08-10_public_cms_and_forms.sql`. | **ready** (Phase 1 MVP) |
| BL-112 | **Connect kiosk / rights & voice** | Same form definitions as BL-111 inside Connect (`/rights-voice`) for office-assisted / named submissions; `channel=connect` + optional participant/staff link. | **ready** (Phase 1 MVP) |
| BL-113 | **Public website branding & media uploads** | **Narrowed 2026-08-22.** Logo, favicon, and brand colours (CSS variables) still deferred. Page images/docs are edited in Admin via `CmsRichTextEditor` + `cms_media` **URL library** (no binary upload). Binary upload / in-editor file library → **BL-119**. | **deferred** |
| BL-119 | **Public website SharePoint media upload** | Upload images/PDFs from the public-website editor and manage that library in-editor. **One SharePoint library** (e.g. `08_Public_Website` or a separate public-web site) shared by **DEV / TEST / PROD** — public files are not env-specific. Must be anonymously readable by yada.org.au visitors (guest/public links, not staff-only). Distinct from BL-051 libraries `01`–`07` (PII / controlled P&P). Do not mix with onboarding/compliance Graph work. Until then, office pastes public URLs. | **discuss** |
...
| ID | Item | Notes | Status |
|----|------|-------|--------|
| BL-065 | **Onboarding — clients, staff, volunteers/carers** | **ALPHA 2026-08-10; Hub inbox 2026-08-21.** Four fixed packs. **Start** from Participants / Staff / Carers. **Inbox** is Hub → Onboarding (drafts, waiting to file, Review due). No DB row until **Save draft** with a name. Delete drafts only; Confirm is the audit lock. Print → wet-sign → Filing location (scan/SharePoint later = BL-051). SQL `docs/sql/2026-08-10_onboarding_cases.sql`. Hub assets on file: profile/consent/support/risk, workforce induction, staff certs, accompanying declaration/WWCC. Dashboard Band 3 **Onboarding review** tile (SQL `docs/sql/2026-08-21_onboarding_review_tile_params.sql`, default yellow 30 / red 0). **Still later:** name search on Hub tab (**BL-118**), scan upload, SharePoint (BL-051), e-sign, floor block on stale consent. Admin requirement catalogue → **BL-126**. Audit gaps → BL-114 · BL-115. | **ready** (Hub inbox; SharePoint/scan follow-on) |
| BL-114 | **Client support plan, risk assessment & communication needs** | **Built 2026-08-21.** Thin artefacts for 0136/0125/0108 (not SIL). Client pack + Care Profile **Support & risk**. Floor **Comms** chips on Check-In / meals when mode or strategies are non-trivial. Hub `client_support_plan` + `client_risk_assessment` (+12 months). SQL: `docs/sql/2026-08-21_client_support_plan.sql`. | **ready** (await DEV+TEST SQL) |
| BL-115 | **Workforce HR evidence (CoC, induction, training, supervision, performance)** | **Locked 2026-08-11** from Audit 2026 `NDISStaffRecordReview` NS: no signed Code of Conduct; no induction records covering privacy/complaints/incidents + safeguarding/abuse response; no ongoing PD; no supervision; no performance review. **Deliver:** CoC policy + signed ack (BL-092); induction checklist in Staff/Volunteer packs (BL-065); thin office training register + supervision log + annual appraisal form; Hub certs remain for WWCC/screening/first aid/driver/med competency. Proportionate to small org — not a full HRIS. | **ready** |
| BL-118 | **Hub Onboarding search** | Name / pack / status / review-due window on Hub → Onboarding. Pack + status dropdowns exist; full search not this slice. | **discuss** |
| BL-126 | **Duty roles + requirement catalogue** | **Built 2026-09-12; floor hooks 2026-09-13.** Catalogue + Duty roles (not ACCESS_ROLES). Floor: meal prep, fleet drive, **on-duty helper check-in**, centre open/close, event open/close, med admin + witness. Empty bind = fall through. Gap = Manager note + PIN. Staff/volunteer certs on `staff_registry`. **Carers fall through** on check-in (no catalogue holds yet). SQL `docs/sql/2026-09-12_duty_roles_requirements.sql` + `docs/sql/2026-09-13_duty_function_hooks.sql`. | **ready** (await 2026-09-13 SQL) |
| BL-127 | **Carer / volunteer Duty-role evidence** | `floor_on_duty` evaluates `staff_registry` only. Carers have no catalogue holds — check-in cannot require NDIS screening / WWCC until `carers_registry` (or shared holds) stores `requirementTypeId`. Volunteers already on staff_registry are covered. | **discuss** |
...
| BL-057 | **Admin full DB backup / restore** | **v2 2026-08-05 — self-learning schema + mode switches.** Each backup discovers tables **and** live public schema (enums/columns/constraints/indexes/RPCs/triggers/RLS) via `export_backup_schema_catalog`. Restore checkboxes: **Apply infrastructure** · **Restore table data** · **Restore login details** (login off = preserve `staff_registry` + env tables). Modes: empty structure promote, data-only, full disaster. SQL: `2026-07-11_backup_restore_rpcs.sql` + `2026-08-05_backup_schema_catalog_rpcs.sql`. Needs `SUPABASE_SERVICE_ROLE_KEY`. Auth passwords (`auth.users`) still out of scope. | **done** |
...
| ID | Item | Completed |
|----|------|-----------|
| BL-123 | Day Centre End of Day Report — calendar, arrivals, meals, checkout, issues | 2026-08-26 |
| BL-116 | In-app App tickets — Raise ticket, Hub tab, Dashboard tile, Postmark notify | 2026-08-20 |
...
| Date | Change |
|------|--------|
| 2026-09-24 | Local working copy confirmed as the only workspace: `C:\Users\Craig\kinship-logistics`. Pickup address book brought across from the OneDrive checkout. |
| 2026-09-23 | Local working copy locked to `C:\Users\Craig\kinship-logistics`. OneDrive checkout is stale. `docs/LOCAL-WORKSPACE.md`. |
| 2026-09-23 | BL-130 Pickup address book — named places, weekday morning/afternoon standing choice, today-only driver change. SQL `2026-09-23_person_addresses.sql`. |
| 2026-09-23 | BL-129 Service exit — off-board clients and staff/volunteers. SQL `2026-09-23_service_exit.sql`. |
| 2026-09-18 | BL-128 Activity log 5W — Where/Why columns + GPS; emergency/close/lockdown/rolls/hops/med-bag/infectious composers; automated sweeps actor System; cap 2000. |
| 2026-09-18 | BL-128 Activity log — auditor-grade detail: named person/place on check-in/out, meal serves, medication doses; compliance assets named; System actor for trigger writes. SQL `2026-09-18_compliance_asset_ledger_summary.sql`. |
| 2026-09-18 | BL-128 Activity log — office record saves (clients, staff, carers, addresses, etc.) write `OFFICE_RECORD_CHANGED`. Floor YELLOW/RED unchanged. Cursor rule `human-activity-log.mdc`. |
| 2026-09-18 | BL-128 Activity log — Admin read-only `operational_ledger` browser (date/type/search). Run Planning IN/OUT writes `RUN_PLANNING_CHANGED`. No extra SQL. Historical Jenny McAlees rows have a time but no actor. |
| 2026-09-13 | Certificate types moved to Lookups → Certificates & orientations; Duty roles + Staff pick only. |
| 2026-09-13 | BL-126 floor hooks — on-duty check-in, centre/event open-close, med admin/witness; SQL `2026-09-13_duty_function_hooks.sql`. Carer evidence = BL-127. |
| 2026-09-12 | BL-126 Duty roles — requirement catalogue, staff assignment, meal prep + Start Run bindings; SQL `2026-09-12_duty_roles_requirements.sql` |
...
| 2026-08-28 | BL-124 Day Centre floor Absent skips Manifest; late arrival + Add Attendee home transport; SQL `2026-08-28_day_centre_floor_absence_home_transport.sql` |
| 2026-08-23 | BL-122 walk-on built — Manifest + Event Deliver doors; guest/client/carer; driver PIN; YELLOW `[WALK-ON]`; SQL `2026-08-23_event_walk_on.sql` |
| 2026-08-23 | BL-122 unplanned walk-on locked — Manifest stop + Event Deliver; guest/client/carer pick-lists; driver PIN; YELLOW office follow-up; no formal leader consult |
| 2026-08-23 | BL-121 one Manifest per run slot — block second Start while live or after close; leftover Disco actives cancelled by SQL |
| 2026-08-22 | BL-120 Event Manage Live tab — office read-only watch of pickups, programme, rolls, issues (`event-watch.ts` + `EventLiveWatchTab`) |
| 2026-08-22 | BL-002 idle PIN lock — `auth_idle_lock_minutes` (default 15; 0 = off); Admin panel; same-staff `PinReauthDialog`; skip active Manifest. SQL `2026-08-22_auth_idle_lock_minutes.sql` |
| 2026-08-22 | Public website WYSIWYG (`CmsRichTextEditor`); `cms_media` URL library in-editor; HTML tab; sanitize on save/render. BL-119 SharePoint public-web upload (one library, all envs). BL-113 narrowed to logo/favicon/colours. |
| 2026-08-21 | Hub Onboarding inbox (no empty drafts, delete until Confirm); Band 3 review tile (30/0 Admin-tunable); floor Comms chips; BL-118 search later |
| 2026-08-21 | BL-114 Client support plan / risk / communication — onboarding + Care Profile + Hub review assets |
| 2026-08-20 | BL-117 day-login operational RLS — anon revoked on PII/ops tables; public CMS + submit_public_form remain; TEST host password runbook |
| 2026-08-20 | BL-116 closed — App tickets + Postmark notify verified on TEST |
| 2026-08-19 | BL-116 App tickets locked (option A) — green Raise ticket + Hub tab; not Maintenance / Big Red lane |
| 2026-08-19 | BL-116 notify — Dashboard App tickets tile + Postmark email (Admin To); not Council mailto |
| 2026-08-11 | BL-092 full DRAFT v0.1 pack written to `05_Policies_Procedures/_Drafts/` (policies, handbooks, BL-114/115 templates) |
| 2026-08-11 | BL-114 client support/risk/comms + BL-115 workforce HR evidence locked from Audit 2026 participant/staff record NS; A/B/C yes; Complaints DOCX next after Incidents draft |
| 2026-08-11 | BL-092 P&P catalogue locked (0136/0125/0108; Connect-primary); office CORE gap scan — CoI, Code of Conduct, RP stance, exit, data-breach proc, handbooks; BL-083 petty cash + debit card; BL-079 belongings locked |
| 2026-08-10 | BL-110 content pack v1 SQL; SHAREPOINT-FOLDER-TAXONOMY.md drafted; BL-051 build + BL-113 still deferred |
| 2026-08-10 | BL-113 website branding/media deferred; BL-051 clarified — SharePoint for office docs, not Vercel disk |
| 2026-08-10 | BL-110/111/112 public web CMS + forms→Hub (yada.org.au / connect.yada.org.au) |
| 2026-08-10 | BL-065 ALPHA onboarding (4 packs, Filing location, Hub +12m review); BL-080 absorbed into consent Hub assets |
| 2026-08-05 | BL-057 restore — FK load window (drop→insert→re-add) + true topo order (fixes trip_legs-before-trips mass skips) |
| 2026-08-05 | BL-057 v2 — backup embeds live schema each run; restore modes: infrastructure / data / login details |
| 2026-07-31 | BL-105 Help — Hub Update Due/Stale SLA + Check-in/roll escalations (Admin-configured times) |
...
| 2026-08-06 | BL-084 global floor announcements — AppShell emergency/MOTD strip; Dashboard fire-alarm panel; Admin `floor_motd`; future med channel on same priority stack |
| 2026-08-02 | BL-106 locked — self-med arrangement; manifest awareness-only; emergency assist → Human Incident; `occurred_at` + structured who (participant + assisting staff) |
