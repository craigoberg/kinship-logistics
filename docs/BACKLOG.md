...
| BL-084 | **Infection control, site close & emergencies** | **Design locked 2026-07-27 (A→B→C).** **Phase A + A.1 done.** **Phase B + C MVP built 2026-07-29** (office has no formal procedures → app is run + evidence). **B:** do-not-open (Start of Day → `closed_no_go`); lockdown/early close (Active Day — block arrivals + orderly close); programme suspend (Event Deliver — gate hop/programme). **C:** Drill\|Live activate + sticky banner + light muster (Expected/Accounted/Missing) + stand-down debrief + Manager PIN on Day Centre / Event Deliver / Manifest. Free-text situation + Yellow\|Red only (no hazard taxonomy). SQL: `docs/sql/2026-07-29_operational_emergencies_mvp.sql`. API: `operational-emergency.ts`. Hub Band 2 tile **Emergency & Site Hold**. Notify = mock SMS (BL-050 later). **Still later:** full Policies/SOPs (BL-092); real SMS; Audit Pack CSV stub if needed. | **ready** (A/A.1/B/C MVP built — smoke + TEST promote) |
