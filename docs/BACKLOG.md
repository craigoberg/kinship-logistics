...
| BL-126 | **Duty roles + requirement catalogue** | **Built 2026-09-12.** Office Admin → Duty roles: catalogue (certificate or orientation; expiry optional), Duty roles (not ACCESS_ROLES), bindings to meal prep / fleet category / one asset. Staff sheet assigns Duty roles and picks requirements from the catalogue. Floor: meal preparer + Start Run use bound requirements; missing/expired = Manager note + PIN (same as BL-073). Centre Open/Close bindable, not seeded. SQL `docs/sql/2026-09-12_duty_roles_requirements.sql`. Medical Admin floor gate later. | **ready** (await DEV+TEST SQL) |
