...
| ID | Item | Notes | Status |
|----|------|-------|--------|
| BL-110 | **Public website + CMS** | Content for `yada.org.au` authored/published in Connect Admin → Public website. Tables: `cms_pages`, `cms_nav`, `cms_media`, publish snapshot. Public routes under `/public/*` (no AppShell / no day-login). Domain cutover: public app on yada.org.au, Connect on connect.yada.org.au. **Content pack v1:** `docs/sql/2026-08-10_public_cms_content_v1.sql` (editorial copy; Contact page has phone/email/address/hours placeholders for office). Branding/uploads = BL-113 (deferred). Policies public subset; full P&P = BL-092 + SharePoint (BL-051). | **ready** (content v1; office to fill contact details) |
...
| BL-113 | **Public website branding & media uploads** | Deferred until content IA settles. Upload/manage: site logo, favicon, hero/gallery images, brand colours (CSS variables). Extends Admin → Public website / `cms_media`. Prefer object storage (Vercel Blob or Supabase Storage) for **public web assets only** — not the office document library. | **deferred** |
...
| ID | Item | Notes | Status |
|----|------|-------|--------|
| BL-065 | **Onboarding — clients, staff, volunteers/carers** | **ALPHA shipped 2026-08-10.** Four fixed packs (Client / Paid Staff / Volunteer / Accompanying). Admin → Onboarding: draft → office Confirm fields → Print filled/blank → wet-sign → **Filing location** text (scan/SharePoint later = BL-051). SQL `docs/sql/2026-08-10_onboarding_cases.sql`. Maps to `participants` / schedules / carers / `staff_registry` certs. Hub assets on file: `client_profile_review`, `client_consent_pack`, `workforce_induction_review`, staff certs, accompanying declaration/WWCC (+12 months or cert expiry). **Review/Update** supersedes prior signed case and resets Hub dates (annual + material change). Linked from Care Profile / Staff / Carer sheets. **Still later:** scan upload, SharePoint (BL-051), e-sign, floor block on stale consent, Admin role→ requirement catalogue builder, Audit Pack `06_` sample rules, automated NDIS plan ingest. **Audit file gaps → BL-114 (client support/risk/comms) · BL-115 (workforce HR evidence).** | **ready** (ALPHA; SharePoint/scan follow-on) |
| BL-114 | **Client support plan, risk assessment & communication needs** | **Locked 2026-08-11** from Audit 2026 `NDISParticipantRecordReview` NS (minor NCs): no org support plan (3.2.1/3.2.4), no risk assessment/review (3.2.2/3.2.3), communication needs/strategies not documented (1.3.2); goals/strengths/needs only partial. **Scope 0136/0125/0108** — thin artefacts, not SIL care-plan suite. Deliver: (1) templates under BL-092 Intake; (2) fields in Client onboarding pack / Care Profile (BL-065); (3) Hub review currency (annual + material change) aligned with consent/profile review. Rights/complaints acknowledgement signature also soft-risk from audit comments. | **ready** |
