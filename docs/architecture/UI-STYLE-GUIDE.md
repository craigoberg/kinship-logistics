...
| **Off-board client or staff** | **Defined** | `ServiceExitDialog` + manager `PinEntryDialog` | Care profile (clients) and Staff sheet | Reason via `MobileOptionButton`. Notes (`CharacterCountedTextarea`, min 20) for Other, Deceased, and every Reactivate. Deceased cannot return. History stays. Does not use guest `archived_at`. Directory defaults to active; Show exited / Show inactive. |
