...
| **Hide chrome on scroll** | **Defined** | `ChromeVisibilityProvider` + `useHideChromeOnScroll` | Dashboard document scroll + Manifest inner pane | Scroll down hides SIM bar, AppShell header, Manifest identity row, BottomNav, and **Cancel / Reset Trip**. Scroll up or back to top reveals. Does **not** hide Close Run, Incident / Raise FABs, SiteNoGo, or MOTD. Manifest listens to the viewport-locked inner scroller — not the document. |
...
|------|---------|----------|
| 2026-09-09 | Hide chrome on scroll | Dashboard + Manifest: scroll down hides SIM / AppShell / BottomNav / Cancel Reset; scroll up reveals. Manifest uses the inner scroller. Close Run and Incident/Raise stay. |
